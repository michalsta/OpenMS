...
protected:
  void registerOptionsAndFlags_() override
  {
    ...
    registerInputFileList_(
      "in_feat", "<files>", StringList(),
      "Optional input featureXML files containing pre-computed features. Bypasses internal feature finding. Must match the number of '-in' files.",
      false, false);
    ...
    registerStringOption_("quantification_method", "<option>", "feature_intensity",
    ...
    registerStringOption_("keep_feature_top_psm_only", "<option>", "true",
                          "If false, also keeps lower ranked PSMs that have the top-scoring"
                          " sequence as a candidate per feature in the same file.",
                          false, true);
    ...
    registerStringOption_("Seeding:charge", "<minChg:maxChg>", "2:5", "Charge range considered for untargeted feature seeds.", false,
                          true); // TODO infer from IDs?
    registerDoubleOption_("Seeding:traceRTTolerance", "<tolerance(sec)>", 3.0,
                          "Combines all spectra in the tolerance window to stabilize identification of isotope patterns. Controls sensitivity (low "
                          "value) vs. specificity (high value) of feature seeds.",
                          false, true); // TODO infer from average MS1 cycle time?
  ...
  ExitCodes centroidAndCorrectPrecursors_(const String& mz_file, MSExperiment& ms_centroided)
  {
    ...
    std::set<Size> corrected_to_highest_intensity_peak
      = PrecursorCorrection::correctToHighestIntensityMS1Peak(ms_centroided,
  ...
  double align_(vector<FeatureMap>& feature_maps, vector<TransformationDescription>& transformations)
  {
    if (feature_maps.size() > 1) // do we have several maps to align / link?
    {
      ...
      for (TransformationDescription& t : transformations)
      {
        ...
        if (t.getDataPoints().size() > 10) { t.fitModel(model_type, model_params); }
  ...
  ExitCodes loadAndCleanupIDFile_(const String& id_file_abs_path,
  ...
  ExitCodes quantifyFraction_(const pair<UInt, std::vector<String>>& ms_files,
                              ...
                              set<String>& variable_modifications)
  {
    ...
    for (String const& mz_file : ms_files.second)
    {
      ...
      if (! targeted_only && in_feat_list.empty())
      {
        ...
        if (debug_level_ > 666)
        {
          FileHandler().storeFeatures("debug_seeds_fraction_" + String(ms_files.first) + "_" + String(fraction_group) + ".featureXML", seeds,
                                      {FileTypes::FEATUREXML}, log_type_);
        }
      }
      ...
      if (! in_feat_list.empty())
      {
        ...
        {
          OPENMS_LOG_WARN << "Primary MS run path in featureXML (" << run_paths[0] << ") does not match the mzML at the same position (" << mz_file
      ...
      if (debug_level_ > 666)
      {
        FileHandler().storeFeatures("debug_fraction_" + String(ms_files.first) + "_" + String(fraction_group) + ".featureXML", feature_maps.back(),
                                    {FileTypes::FEATUREXML}, log_type_);
      }
  ...
  ExitCodes inferProteinGroups_(ConsensusMap& consensus, const set<String>& fixed_modifications)
  {
  ...
  ExitCodes main_(int, const char**) override
  {
    ...
    if (! in_feat.empty() && in_feat.size() != in.size())
    {
      throw Exception::InvalidParameter(__FILE__, __LINE__, OPENMS_PRETTY_FUNCTION,
                                        "Number of featureXML files (-in_feat, " + String(in_feat.size()) + ") must match number of mzML files (-in, "
                                          + String(in.size()) + ").");
    }
